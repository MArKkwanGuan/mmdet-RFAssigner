...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    assert mode in ['iou', 'iof', 'giou', 'gcd', 'cwd', 'nwd', 'wd', 'kld'], f'Unsupported mode {mode}'
    ...
    if mode in ['gcd', 'cwd', 'nwd', 'wd', 'kld']:
        if is_aligned:
            center1 = (bboxes1[..., :2] + bboxes1[..., 2:]) / 2
            center2 = (bboxes2[..., :2] + bboxes2[..., 2:]) / 2
            whs = fp16_clamp(center2 - center1, min=0)
            w1 = fp16_clamp(bboxes1[..., 2] - bboxes1[..., 0], min=0)
            h1 = fp16_clamp(bboxes1[..., 3] - bboxes1[..., 1], min=0)
            w2 = fp16_clamp(bboxes2[..., 2] - bboxes2[..., 0], min=0)
            h2 = fp16_clamp(bboxes2[..., 3] - bboxes2[..., 1], min=0)
        else:
            center1 = (bboxes1[..., :, None, :2] + bboxes1[..., :, None, 2:]) / 2
            center2 = (bboxes2[..., None, :, :2] + bboxes2[..., None, :, 2:]) / 2
            whs = fp16_clamp(center2 - center1, min=0)
            w1 = fp16_clamp(bboxes1[..., :, None, 2] - bboxes1[..., :, None, 0], min=0)
            h1 = fp16_clamp(bboxes1[..., :, None, 3] - bboxes1[..., :, None, 1], min=0)
            w2 = fp16_clamp(bboxes2[..., None, :, 2] - bboxes2[..., None, :, 0], min=0)
            h2 = fp16_clamp(bboxes2[..., None, :, 3] - bboxes2[..., None, :, 1], min=0)
        ...
        if mode == 'cwd':
            center_distance = (whs[..., 0] ** 2 / (w1 * w2  + eps)) + (whs[..., 1] ** 2 / (h1 * h2 + eps))
            wh_distance = (((w1 - w2) ** 2 / (w1 * w2 + eps))  + ((h1 - h2)  ** 2 / (h1 * h2 + eps))) / 4
        ...
        if mode == 'wd':
            center_distance = whs[..., 0] ** 2 + whs[..., 1] ** 2
        ...
        if mode == 'kld':
            kl=(w2**2/(w1**2 + eps)+h2**2/(h1**2 + eps)+4*whs[..., 0]**2/(w1**2 + eps)+4*whs[..., 1]**2/(h1**2 + eps)+torch.log(w1**2/(w2**2 + eps))+torch.log(h1**2/(h2**2 + eps))-2)/2
            kld = 1 / (1 + kl)
            return kld
